...
@section('content')
                ...
                <ul class="mb-0 small">
                    @foreach ($resetRequests as $r)
                        <li>
                            ...
                            <form action="{{ route('admin.users.reset-password', $r->id) }}" method="post"
                                class="d-inline ms-2" onsubmit="return confirm('Reset password untuk {{ $r->name }}?')">
                                @csrf
                                <button class="btn btn-sm btn-outline-dark">Reset sekarang</button>
                            </form>
                            <a class="btn btn-sm btn-outline-secondary ms-1"
                                href="{{ route('admin.users.index', ['q' => $r->identifier]) }}">Lihat di Manajemen User</a>
                        </li>
                    @endforeach
                </ul>
            </div>
            <a class="btn btn-sm btn-outline-secondary"
                href="{{ route('admin.users.index', ['role' => 'guru']) }}">Kelola</a>
        </div>
    @endif
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3">
            <a href="{{ route('admin.users.index') }}" class="text-decoration-none text-reset">
                <div class="card h-100"
                    style="background: rgba(30, 58, 138, 0.08); backdrop-filter: blur(20px); border: 1px solid rgba(30, 58, 138, 0.15);">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <p class="text-muted small mb-1">Total Users</p>
                                <h3 class="mb-0" style="color: #1e3a8a;">{{ $stats['total_users'] }}</h3>
                                <small class="text-muted">
                                    {{ $stats['total_siswa'] }} siswa, {{ $stats['total_guru'] }} guru
                                </small>
                            </div>
                            <div class="fs-1" style="color: #1e3a8a; opacity: 0.2;">
        ...
        <div class="col-md-3">
            <a href="{{ route('admin.dashboard.login-history', ['date_from' => now()->format('Y-m-d'), 'date_to' => now()->format('Y-m-d')]) }}"
                class="text-decoration-none text-reset">
                <div class="card h-100"
                    style="background: rgba(252, 231, 243, 0.5); backdrop-filter: blur(20px); border: 1px solid rgba(236, 72, 153, 0.2);">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <p class="text-muted small mb-1">Login Hari Ini</p>
                                <h3 class="mb-0" style="color: #ec4899;">{{ $stats['today_logins'] }}</h3>
                                <small class="text-muted">aktivitas pengguna</small>
                            </div>
                            <div class="fs-1" style="color: #ec4899; opacity: 0.2;">
        ...
        <div class="col-md-3">
            <a href="{{ route('admin.dashboard.refleksi-history', ['date_from' => now()->format('Y-m-d'), 'date_to' => now()->format('Y-m-d')]) }}"
                class="text-decoration-none text-reset">
                <div class="card h-100"
                    style="background: rgba(59, 130, 246, 0.1); backdrop-filter: blur(20px); border: 1px solid rgba(59, 130, 246, 0.2);">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <p class="text-muted small mb-1">Refleksi Hari Ini</p>
                                <h3 class="mb-0" style="color: #3b82f6;">
                                    {{ $stats['today_refleksi_siswa'] + $stats['today_refleksi_guru'] }}</h3>
                                <small class="text-muted">
                                    {{ $stats['today_refleksi_siswa'] }} siswa, {{ $stats['today_refleksi_guru'] }} guru
                                </small>
                            </div>
                            <div class="fs-1" style="color: #3b82f6; opacity: 0.2;">
        ...
        <div class="col-md-3">
            <a href="{{ route('admin.dashboard.mood-history', ['date_from' => now()->format('Y-m-d'), 'date_to' => now()->format('Y-m-d')]) }}"
                class="text-decoration-none text-reset">
                <div class="card h-100"
                    style="background: rgba(244, 114, 182, 0.1); backdrop-filter: blur(20px); border: 1px solid rgba(244, 114, 182, 0.25);">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <p class="text-muted small mb-1">Mood Tracking Hari Ini</p>
                                <h3 class="mb-0" style="color: #f472b6;">{{ $stats['today_mood_tracking'] }}</h3>
                                <small class="text-muted">entri pemantauan emosi</small>
                            </div>
                            <div class="fs-1" style="color: #f472b6; opacity: 0.2;">
                                ...
                                @forelse($dailyTrend as $trend)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            ...
                                            <div class="progress"
                                                style="height: 5px; width: 100px; display: inline-block; margin-left: 10px;">
                                                <div class="progress-bar bg-primary"
                                                    style="width: {{ min(100, ($trend->count / max($dailyTrend->max('count'), 1)) * 100) }}%">
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center text-muted py-3">Belum ada analisis</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                        ...
                        <table class="table table-sm table-hover mb-0">
                            ...
                            <tbody>
                                @forelse($activeBookings as $booking)
                                    <tr>
                                        <td class="small">
                                            {{ optional(optional($booking->siswaKelas)->siswa->user)->name ?? '-' }}</td>
                                        <td class="small">
                                            @if($booking->slot && $booking->slot->guru && $booking->slot->guru->user)
                                                {{ $booking->slot->guru->user->name }}
                                            @else
                                                -
                                            @endif
                                        </td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center text-muted py-3">Belum ada booking aktif
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
