        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
            <div>
                ...
                <div class="text-muted small">
                    {{ $guruJenis === 'wali_kelas' ? 'Data khusus untuk kelas Anda' : 'Data seluruh siswa' }}</div>
            </div>
            <div>
                ...
                @if ($guruJenis === 'bk')
                    <div class="row g-3 align-items-end mt-3 filter-row" id="rowKelas">
                        <div class="col-12 col-md-3">
                            ...
                            <input type="text" class="form-control form-control-sm mb-1" id="kelas-search2"
                ...
                <div class="row g-3 align-items-end mt-3 filter-row" id="rowSiswa">
                    @if ($guruJenis === 'bk')
                        <div class="col-12 col-md-3">
                            ...
                            <input type="text" class="form-control form-control-sm mb-1" id="kelas-search"
                                placeholder="Cari kelas...">
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <input type="text" class="form-control form-control-sm mb-1" id="siswa-search2"
                        ...
                        <div class="col-12 col-md-6">
                            <div class="d-flex align-items-center justify-content-between mb-2">
                                ...
                                <div class="d-flex gap-2 align-items-center">
                                    <select id="detailKelasFilter" class="form-select form-select-sm"
...
@push('scripts')
    ...
    <script>
        (function() {
            ...
            const kelasSel = $('#kelasSel'); // Dropdown kelas di Baris 3 (Siswa) untuk BK
            const siswaSel = $('#siswaSel'); // Dropdown siswa di Baris 3 (Siswa)
            const kelasSearch = $('#kelas-search');
            const siswaSearch = $('#siswa-search');
            // Timeline rows
            const kelasSel2 = $('#kelasSel2'); // Dropdown kelas di Baris 2 (Kelas)
            ...
            const siswaSel2 = $('#siswaSel2'); // Dropdown siswa di Baris 3 (Siswa) - jika ada elemen duplikat ID
            ...
            function render(labels, values, colors, axisMode) {
                ...
                const options = isVertical ? {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: (ctx) => ` ${scoreEmoji(ctx.parsed.x)}  avg ${ctx.parsed.x}`
                            }
                        }
                    }
                };
                chart = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            data: values,
                            backgroundColor: (colors && colors.length === values.length) ? colors :
                                values.map(scoreToColor),
                            borderWidth: 0,
                            borderRadius: 6
                        }]
                    },
            ...
            async function loadKelasTimeline() {
                ...
                @if ($guruJenis === 'bk')
                    ...
                    if (detailKelasFilter) {
                        detailKelasFilter.value = sel.value;
                    }
                    await loadDetailKelas((kelasPeriodSel?.value) || 'daily', kelasFromEl?.value || data.from);
                    await loadDetailSiswa((kelasPeriodSel?.value) || 'daily', kelasFromEl?.value || data.from, sel
                        .value);
                @endif
            }
            ...
            async function updateSiswaDropdown(kelasId, targetSiswaSel) {
                if (!kelasId || !targetSiswaSel) return;
                const url = `{{ route('guru.tren_emosi.siswa') }}?kelas_id=${encodeURIComponent(kelasId)}`;
                try {
                    ...
                    const opts = (data.items || []).map(it => `<option value="${it.id}">${it.label}</option>`).join(
                        '');
                    targetSiswaSel.innerHTML = `<option value="">— Pilih siswa —</option>` + opts;
                } catch (e) {
                    console.error("Gagal memuat siswa:", e);
                }
            }

            // Saat kelas dipilih di baris 2, update dropdown siswa di baris 3
            if (kelasSel2) {
                kelasSel2.addEventListener('change', async () => {
                    // Update dropdown siswa di baris 3 (siswaSel2 atau siswaSel)
                    const targetSiswa = siswaSel2 || siswaSel;

                    // Jika dropdown siswa ada, update isinya
                    if (targetSiswa) {
                        await updateSiswaDropdown(kelasSel2.value, targetSiswa);

                        // Opsional: Jika dropdown kelas di baris 3 ada dan berbeda elemennya, sinkronkan nilainya
                        if (kelasSel && kelasSel !== kelasSel2) {
                            kelasSel.value = kelasSel2.value;
                        }
                    }
                });
            }

            // --- PERBAIKAN: Event Listener untuk Dropdown Kelas Baris 3 (Timeline Siswa) ---
            // Saat kelas dipilih di baris 3, update dropdown siswa di baris 3 itu sendiri
            if (kelasSel) {
                kelasSel.addEventListener('change', async () => {
                    const targetSiswa = siswaSel2 || siswaSel;
                    if (targetSiswa) {
                        await updateSiswaDropdown(kelasSel.value, targetSiswa);
                    }
                });
            }

            btnLoadKelas?.addEventListener('click', () => {
                setMode('kelas');
                loadKelasTimeline();
            });
            kelasPeriodSel?.addEventListener('change', () => {
                setMode('kelas');
                loadKelasTimeline();
            });
            btnLoadSiswa?.addEventListener('click', () => {
                setMode('siswa');
                loadSiswaTimeline();
            });
            siswaPeriodSel?.addEventListener('change', () => {
                setMode('siswa');
                loadSiswaTimeline();
            });
            (siswaSel2 || siswaSel)?.addEventListener('change', () => {
                /* wait for click */
            });
            ...
            @if ($guruJenis === 'bk')
                async function loadDetailKelas(period, from) {
                    ...
                    p.set('detail', '1');
                    const res = await fetch(`{{ route('guru.tren_emosi.data') }}?${p.toString()}`, {
                        headers: {
                            'Accept': 'application/json'
                        }
                    });
